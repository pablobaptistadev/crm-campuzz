import { describe, expect, it } from 'vitest';

import { moneyFromCents, moneyFromUnits } from 'src/financeiro/core/domain/value-objects/money.value-object';
import {
  mapSubscription,
  mapTransaction,
  totalValueOf,
} from 'src/financeiro/gateways/routerfy/routerfy-contract.mapper';
import { unwrapList } from 'src/financeiro/gateways/routerfy/routerfy-payload.types';

describe('unwrapList', () => {
  it('aceita os dois formatos que a Routerfy usa para lista', () => {
    expect(unwrapList([{ id: 'a' }])).toEqual([{ id: 'a' }]);
    expect(unwrapList({ data: [{ id: 'a' }] })).toEqual([{ id: 'a' }]);
    expect(unwrapList({})).toEqual([]);
    expect(unwrapList(null)).toEqual([]);
  });
});

describe('mapSubscription', () => {
  // Conferido contra a producao: a assinatura dtg1wflupudl3ntyusm5dv794y vem com
  // `amount: 54700` e vale R$ 547,00. O teste antigo dizia "reais" e passava
  // porque concordava com o codigo, nao com a Routerfy — e uma parcela de 547
  // virava 54.700 na tela.
  it('le valores em centavos, como a Routerfy manda', () => {
    const contract = mapSubscription({
      id: 'sub-1',
      items: [{ amount: 54700 }],
      invoices: [{ id: 'inv-1', amount: 54700 }],
    });

    expect(contract.amount).toEqual(moneyFromUnits(547));
    expect(contract.invoices[0]?.amount).toEqual(moneyFromUnits(547));
  });

  // A assinatura nao traz `amount` no topo; ler o campo que nao existe dava R$ 0
  // num contrato de R$ 547 por mes.
  it('tira o valor da assinatura dos itens quando nao ha amount no topo', () => {
    expect(
      mapSubscription({ items: [{ amount: 54700 }] }).amount,
    ).toEqual(moneyFromCents(54700));
  });

  // A Routerfy nao manda a data da proxima cobranca; o vencimento em aberto mais
  // proximo e a mesma informacao, e deixar em branco esconde o que todo mundo
  // procura primeiro no painel.
  it('deriva a proxima cobranca do vencimento em aberto mais proximo', () => {
    const contract = mapSubscription({
      invoices: [
        { id: 'a', amount: 1, status: 'paid', dueAt: '2026-07-06T00:00:00.000Z' },
        { id: 'c', amount: 1, status: 'pending', dueAt: '2026-11-05T00:00:00.000Z' },
        { id: 'b', amount: 1, status: 'pending', dueAt: '2026-10-05T00:00:00.000Z' },
      ],
    });

    expect(contract.nextChargeAt).toBe('2026-10-05T00:00:00.000Z');
  });

  it('traduz o metodo de pagamento, e mantem o desconhecido como veio', () => {
    expect(
      mapSubscription({ paymentMethods: [{ paymentMethod: 'credit_card' }] })
        .paymentMethod,
    ).toBe('Cartao de credito');
    expect(
      mapSubscription({ paymentMethods: [{ paymentMethod: 'cripto' }] })
        .paymentMethod,
    ).toBe('cripto');
  });

  it('so aceita frequencia que conhece', () => {
    expect(mapSubscription({ frequency: 'month' }).frequency).toBe('month');
    expect(mapSubscription({ frequency: 'decada' }).frequency).toBeNull();
  });

  it('da a fatura sem id uma chave estavel em vez de descarta-la', () => {
    const contract = mapSubscription({
      invoices: [{ code: 'FAT-1' }, {}],
    });

    expect(contract.invoices[0]?.externalInvoiceId).toBe('FAT-1');
    expect(contract.invoices[1]?.externalInvoiceId).toBe('sem-id-1');
  });

  it('aguenta um payload vazio sem quebrar', () => {
    const contract = mapSubscription({});

    expect(contract.kind).toBe('SUBSCRIPTION');
    expect(contract.status).toBe('unknown');
    expect(contract.invoices).toEqual([]);
    expect(contract.customer.email).toBeNull();
  });
});

describe('mapTransaction', () => {
  it('sintetiza uma fatura para a venda a vista nao ficar sem financeiro', () => {
    const contract = mapTransaction({
      id: 'trx-1',
      code: 'T-1',
      status: 'paid',
      total: 500,
      paidAt: '2026-03-01T00:00:00.000Z',
    });

    expect(contract.kind).toBe('TRANSACTION');
    expect(contract.transactionId).toBe('trx-1');
    expect(contract.subscriptionId).toBeNull();
    expect(contract.invoices).toHaveLength(1);
    expect(contract.invoices[0]?.amount).toEqual(moneyFromCents(500));
  });

  it('prefere o total ao amount quando os dois vem', () => {
    expect(mapTransaction({ amount: 100, total: 120 }).amount).toEqual(
      moneyFromCents(120),
    );
  });
});

// Numeros reais da assinatura 2026071000000304, conferidos contra o painel:
// 2 pagas (R$ 1.641), 2 canceladas (R$ 1.094), 10 agendadas (R$ 5.470).
// O painel mostra R$ 7.111 e proximo pagamento 05/10 — as canceladas ficam de
// fora dos dois.
describe('faturas canceladas', () => {
  const contrato = mapSubscription({
    invoices: [
      { id: 'a', amount: 54700, status: 'paid', dueAt: '2026-07-06T00:00:00.000Z' },
      { id: 'b', amount: 54700, status: 'canceled', dueAt: '2026-08-05T00:00:00.000Z' },
      { id: 'c', amount: 54700, status: 'canceled', dueAt: '2026-09-08T00:00:00.000Z' },
      { id: 'd', amount: 109400, status: 'paid', dueAt: '2026-09-17T00:00:00.000Z' },
      { id: 'e', amount: 54700, status: 'scheduled', dueAt: '2026-10-05T00:00:00.000Z' },
    ],
  });

  it('nao soma no total o que foi cancelado', () => {
    expect(totalValueOf(contrato)).toEqual(moneyFromCents(54700 + 109400 + 54700));
  });

  it('nao aponta uma cancelada como proxima cobranca', () => {
    expect(contrato.nextChargeAt).toBe('2026-10-05T00:00:00.000Z');
  });

  // A coluna e um SELECT: o `paid` cru da Routerfy derrubava a vinculacao com
  // `invalid input value for enum`, e por isso nenhum contrato chegou a ser
  // gravado. Quem sai daqui ja tem de estar no vocabulario do dominio.
  it('entrega as faturas ja traduzidas para o dominio', () => {
    expect(contrato.invoices.map((fatura) => fatura.status)).toEqual([
      'PAID',
      'CANCELED',
      'CANCELED',
      'PAID',
      'PENDING',
    ]);
  });
});

describe('totalValueOf', () => {
  it('soma as faturas, que e onde desconto e acrescimo aparecem', () => {
    const contract = mapSubscription({
      amount: 100,
      invoices: [{ id: 'a', amount: 90 }, { id: 'b', amount: 100 }],
    });

    expect(totalValueOf(contract)).toEqual(moneyFromCents(190));
  });

  it('sem faturas, cai no valor do contrato', () => {
    expect(totalValueOf(mapSubscription({ amount: 100 }))).toEqual(
      moneyFromCents(100),
    );
  });
});
